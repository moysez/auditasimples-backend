...
def _fmt_percent(x: Any) -> str:
    """
    Aceita fração (0.10) ou percentual (10). Detecta e normaliza.
    """
    try:
        n = float(x or 0.0)
    except Exception:
...
def gerar_relatorio_fiscal(totals: Dict[str, Any], client_name: str = "Cliente", cnpj: str = "00.000.000/0000-00") -> str:
    ...
    st_corretos   = totals.get("st_cfop_csosn_corretos", 0) or 0
    st_incorretos = totals.get("st_incorreta", 0) or 0
    mono_sem_ncm  = totals.get("monofasico_sem_ncm", 0) or 0
    mono_desc     = totals.get("monofasico_palavra_chave", 0) or 0
    ...
    p = doc.add_paragraph(f"{client_name}  |  CNPJ: {cnpj}"); p.runs[0].bold = True; p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p2 = doc.add_paragraph(f"Período auditado: {period_start} — {period_end}"); p2.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p3 = doc.add_paragraph(f"Data da análise: {data_hoje}"); p3.alignment = WD_ALIGN_PARAGRAPH.CENTER
    doc.add_paragraph("")

    # 1. Resumo Geral
    doc.add_heading("1. Resumo Geral", level=1)
    t_resumo = doc.add_table(rows=1, cols=2); t_resumo.style = "Table Grid"
    t_resumo.cell(0,0).text="Indicador"; t_resumo.cell(0,1).text="Valor"
    ...
    _format_table_borders(t_resumo); doc.add_paragraph("")

    # 2. Resumo Tributário
    doc.add_heading("2. Resumo Tributário", level=1)
    t_trib = doc.add_table(rows=1, cols=2); t_trib.style = "Table Grid"
    t_trib.cell(0,0).text="Descrição"; t_trib.cell(0,1).text="Valor"
    ...
    _format_table_borders(t_trib); doc.add_paragraph("")

    # 3. Erros Fiscais
    doc.add_heading("3. Erros Fiscais", level=1)
    t_err = doc.add_table(rows=1, cols=2); t_err.style = "Table Grid"
    t_err.cell(0,0).text="Indicador"; t_err.cell(0,1).text="Quantidade"
    for c in t_err.rows[0].cells: c.paragraphs[0].runs[0].bold = True
    ...
    _format_table_borders(t_err); doc.add_paragraph("")

    # 4. Itens Deduplicados (apenas monofásicos)
    if produtos_duplicados:
        doc.add_heading("4. Itens Deduplicados (por descrição)", level=1)
        t_dup = doc.add_table(rows=1, cols=4); t_dup.style = "Table Grid"
        hdr = t_dup.rows[0].cells
        hdr[0].text = "Código"; hdr[1].text = "Descrição"; hdr[2].text = "Ocorrências"; hdr[3].text = "Valor Total"
        ...
        _format_table_borders(t_dup); doc.add_paragraph("")

    # 5. Detalhamento Analítico (monofásicos com ST incorreto)
    produtos_excluidos = []
    ...
    if produtos_excluidos:
        ...
        for mes, lista in sorted(grupos.items()):
            ...
            tabela = doc.add_table(rows=1, cols=12); tabela.style = "Table Grid"
            headers = ["Data","Documento","Código","Descrição","NCM","CFOP","CST/CSOSN","NCM/CEST Recomendado","Qtd","Vlr Unit","Vlr Total","Chave"]
            for i,h in enumerate(headers):
                tabela.cell(0,i).text = h
                tabela.cell(0,i).paragraphs[0].runs[0].bold = True
            ...
            for it in lista:
                ...
                r[6].text = str(it.get("csosn") or it.get("cst") or "-")
    ...
    p1 = doc.add_paragraph(f"💰 R$ {_fmt_money(economia)} por mês (estimado)"); p1.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p2 = doc.add_paragraph(f"📅 Em um ano: R$ {_fmt_money(economia_anual)}"); p2.alignment = WD_ALIGN_PARAGRAPH.CENTER

    # 8. Próximos Passos
    doc.add_heading("8. Próximos Passos", level=1)
    for s in ["1. Conferir dados no PGDAS-D.","2. Retificar períodos passados (se aplicável).","3. Preparar pedido de restituição/compensação.","4. Acompanhar até o reembolso."]:
        doc.add_paragraph(s)
